...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Amdr600Core"

  it should "execute VLIW packets of LD, ST, ADD, HALT instructions correctly to perform vector addition" in {
    ...
    val hexFile = findWorkspaceFile("amdr600/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Amdr600Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("30000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
