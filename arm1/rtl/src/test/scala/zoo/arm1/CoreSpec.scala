...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Arm1Core"

  it should "execute LDR, STR, ADD, HALT instructions correctly to perform vector addition" in {
    ...
    val hexFile = findWorkspaceFile("arm1/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Arm1Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("100000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
