...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Bullgamma60Core"

  it should "execute LD, ADD, ST, FORK, JOIN, and HLT correctly with two time-multiplexed threads" in {
    ...
    val hexFile = findWorkspaceFile("bullgamma60/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Bullgamma60Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("1c0000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
