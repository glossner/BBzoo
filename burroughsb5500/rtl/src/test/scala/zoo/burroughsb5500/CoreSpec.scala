...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "B5500Core"

  it should "execute PUSH, POP, ADD and SUB correctly using stack" in {
    ...
    val hexFile = findWorkspaceFile("burroughsb5500/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new B5500Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("60000000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
