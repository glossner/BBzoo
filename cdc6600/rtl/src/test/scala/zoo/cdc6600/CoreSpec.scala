...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Cdc6600Core"

  it should "execute address and register arithmetic correctly" in {
    ...
    val hexFile = findWorkspaceFile("cdc6600/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Cdc6600Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("100000000000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
