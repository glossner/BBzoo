...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Cdc6600ppuCore"

  it should "execute LD, ADD, ST, and HLT correctly to perform vector addition" in {
    ...
    val hexFile = findWorkspaceFile("cdc6600ppu/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Cdc6600ppuCore) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("180", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
