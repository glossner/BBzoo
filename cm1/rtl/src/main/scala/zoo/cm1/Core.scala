  ...
  switch(state) {
    ...
    is(sDecode) {
      when(decoder.io.hlt) {
        ...
        insts := insts + 1.U
        state := sFetch
      }.elsewhen(decoder.io.route_cm1) {
        val prevAccs = Wire(Vec(4, Bool()))
        for (i <- 0 until 4) {
          prevAccs(i) := pe_accs(i)
        }
        when(decoder.io.dim === 0.U) {
          pe_accs(0) := prevAccs(1)
          pe_accs(1) := prevAccs(0)
          pe_accs(2) := prevAccs(3)
          pe_accs(3) := prevAccs(2)
        }.otherwise {
          pe_accs(0) := prevAccs(2)
          pe_accs(2) := prevAccs(0)
          pe_accs(1) := prevAccs(3)
          pe_accs(3) := prevAccs(1)
        }
        pc := pc + 1.U
        insts := insts + 1.U
        state := sFetch
