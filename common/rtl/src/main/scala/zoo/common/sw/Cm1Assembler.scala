...
class Cm1Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def getArchInstructionSize(line: String): Int = {
    ...
    if (mnemonic.startsWith("ROUTE_CM1")) return 1
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (mnemonic == "ROUTE_CM1") {
      val dim = parseNumeric(parts(1))
      return Seq(0x7000 | (dim & 0x1))
    }
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown CM-1 instruction: $line")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled CM-1 Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
