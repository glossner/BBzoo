...
class Cydra5Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (upper.startsWith("BR_ROT ")) {
      val target = parseNumericOrSymbol(s.substring(7))
    ...
    val slots = s.split(";")
    if (slots.length != 3) {
      throw new Exception(s"Cydra 5 instruction must have exactly 3 slots separated by semicolons: $line")
    }
    ...
    if (ctrlStr != "NOP") {
      ...
      val target = if (op == "BR_ROT") parseNumericOrSymbol(parts(1)) else 0
