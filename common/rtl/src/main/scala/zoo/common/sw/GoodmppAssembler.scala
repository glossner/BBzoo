...
class GoodmppAssembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def getArchInstructionSize(line: String): Int = {
    ...
    if (mnemonic == "CLR_CARRY" || mnemonic == "LD_PE_MASK") return 1
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (mnemonic == "LD_PE_MASK") return Seq(0x7000)
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown Goodyear MPP instruction: $line")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Goodyear MPP Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
