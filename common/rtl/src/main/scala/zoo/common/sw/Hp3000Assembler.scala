...
class Hp3000Assembler extends BaseAssembler with StackArchitecture with VariableLengthFormat with FixedPointOperations {
  commentPattern = "(;|//).*"
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (mnemonic == "ADD") return Seq(0x60 << 8)
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown HP 3000 instruction: $s")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled HP 3000 Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
