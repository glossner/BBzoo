...
class Ibm801Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  def parseReg(regStr: String): Int = {
    ...
    if (s.startsWith("R")) {
      ...
      throw new Exception(s"Invalid IBM 801 register: $regStr")
    }
  }
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    throw new Exception(s"Unknown IBM 801 instruction: $line")
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled IBM 801 Hex File"
    for (v <- values) {
