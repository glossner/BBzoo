...
class IbmS370VfAssembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def formatOutput(values: Seq[Int]): String = {
    ...
    out += "# Compiled IBM S/370 VF Hex File"
    for (v <- values) {
