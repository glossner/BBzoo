...
class Intel3002Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown Intel 3002 instruction: $line")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Intel 3002 Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
