...
class Mali200Assembler extends BaseAssembler with VectorArchitecture with VariableLengthFormat with FixedPointOperations {
  commentPattern = "(;|//).*"
  ...
  override def getArchInstructionSize(line: String): Int = {
    ...
    if (mnemonic == "HALT" || mnemonic == "VADD") return 1
    if (mnemonic == "VLD" || mnemonic == "VST") return 2
    1
  }
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    mnemonic match {
      ...
      case "VADD" =>
        ...
        Seq((0x20 << 24) | (destIdx << 22) | (src1Idx << 18) | (src2Idx << 14))

      case _ =>
        throw new Exception(s"Unknown Mali200 instruction: $s")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Mali200 Hex File"
    for (v <- values) {
