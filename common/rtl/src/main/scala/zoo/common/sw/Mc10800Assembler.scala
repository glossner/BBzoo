...
class Mc10800Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown Motorola 10800 instruction: $line")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Motorola 10800 Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
