...
class NecSx2Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (op == "VLD") {
      ...
      val rs = parseReg(operands(2))
      return Seq((2 << 28) | (vk << 24) | (vi << 20) | (rs << 16))
    } else if (op == "VADD") {
      val vk = parseVecReg(operands(0))
      val vi = parseVecReg(operands(1))
      val vj = parseVecReg(operands(2))
      return Seq((1 << 28) | (vk << 24) | (vi << 20) | (vj << 16))
    } else {
      throw new Exception(s"Unknown NEC SX-2 instruction: $line")
    }
  }
