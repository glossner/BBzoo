...
class Symbolics3600Assembler extends BaseAssembler with StackArchitecture with VariableLengthFormat with FixedPointOperations {
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    throw new Exception(s"Unknown Symbolics 3600 instruction: $line")
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Symbolics 3600 Hex File"
    for (v <- values) {
