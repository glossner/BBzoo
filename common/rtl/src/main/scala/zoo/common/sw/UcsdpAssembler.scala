...
class UcsdpAssembler extends BaseAssembler with StackArchitecture with VariableLengthFormat with FixedPointOperations {
  commentPattern = "(;|//).*"
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    if (mnemonic == "ADD") return Seq(0xA0 << 8)
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown UCSD Pascal P-Machine instruction: $s")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled UCSD Pascal Hex File"
    for (v <- values) {
      out += f"${v & 0xFFFF}%04X"
    }
    out.mkString("\n") + "\n"
