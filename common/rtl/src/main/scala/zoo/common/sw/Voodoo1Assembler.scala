...
class Voodoo1Assembler extends BaseAssembler with GeneralRegisterArchitecture with VariableLengthFormat with FixedPointOperations {
  commentPattern = "(;|//).*"
  ...
  override def getArchInstructionSize(line: String): Int = {
    ...
    if (mnemonic == "HALT" || mnemonic == "ADD") return 1
  ...
  override def assembleInstruction(line: String): Seq[Int] = {
    ...
    mnemonic match {
      ...
      case _ =>
        throw new Exception(s"Unknown Voodoo1 instruction: $s")
    }
  }

  override def formatOutput(values: Seq[Int]): String = {
    val out = mutable.Buffer[String]()
    out += "# Compiled Voodoo1 Hex File"
    for (v <- values) {
