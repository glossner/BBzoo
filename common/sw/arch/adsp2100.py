...
class Adsp2100Assembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 16
    address_width = 16
    num_gprs = 16
    num_fprs = 0
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "ADD":
            # Expecting "ADD AR"
            if len(parts) < 2 or parts[1].strip().upper() != "AR":
                raise Exception(f"Invalid operands for ADSP-2100 ADD: {line}")
            return [0x22 << 8]
        ...
        if mnemonic == "LD":
            ...
            if reg == "AX0":
                ...
                raise Exception(f"Invalid register for LD in ADSP-2100: {reg}")
            ...
            if reg == "AR":
                ...
                raise Exception(f"Invalid register for ST in ADSP-2100: {reg}")

        else:
            raise Exception(f"Unknown ADSP-2100 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled ADSP-2100 Hex File")
        for val in values:
