import re
from zoo_assembler import BaseAssembler

from architecture.storage_hierarchy import AccumulatorArchitecture
from architecture.instruction_format import FixedLengthFormat
from architecture.operations import FixedPointOperations

class Bullgamma60Assembler(BaseAssembler, AccumulatorArchitecture, FixedLengthFormat, FixedPointOperations):
    word_width = 24
    address_width = 16
    has_link_bit = False
    op_code_width = 6
    instruction_width = 24
    has_multiply_divide = False

    def get_arch_instruction_size(self, line):
        return 1

    def parse_numeric_or_symbol(self, val):
        val = val.strip()
        if val in self.symbols:
            return self.symbols[val]
        return self.parse_numeric(val)

    def assemble_instruction(self, line):
        line = line.strip()
        if not line:
            return []

        parts = line.split()
        mnemonic = parts[0].upper()

        if mnemonic == "HLT":
            return [6 << 18]
        if mnemonic == "JOIN":
            return [5 << 18]
        if mnemonic == "FORK":
            addr_str = parts[1].strip()
            addr = self.parse_numeric_or_symbol(addr_str)
            return [(4 << 18) | (addr & 0xFFFF)]

        if mnemonic in ["LD", "ADD", "ST"]:
            op = 1 if mnemonic == "LD" else 2 if mnemonic == "ADD" else 3
            reg_str = parts[1].replace(',', '').strip().upper()
            addr_str = parts[2].strip()
            
            reg_val = 0
            if reg_str == "ACC1":
                reg_val = 1
            elif reg_str == "ACC0":
                reg_val = 0
            else:
                try:
                    reg_val = self.parse_numeric_or_symbol(reg_str)
                except ValueError:
                    pass
            
            addr = self.parse_numeric_or_symbol(addr_str)
            return [(op << 18) | ((reg_val & 3) << 16) | (addr & 0xFFFF)]

        try:
            return [self.parse_numeric_or_symbol(line)]
        except ValueError:
            raise Exception(f"Unknown Bull Gamma 60 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled Bull Gamma 60 Hex File")
        for val in values:
            out.append(f"{val & 0xFFFFFF:06X}")
        return "\n".join(out) + "\n"
