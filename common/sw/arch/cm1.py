...
def assemble(input_text):
    ...
    for line in lines:
        ...
        if mnemonic in ["HALT", "HLT", "CLR_CARRY"]:
            current_addr += 1
        elif mnemonic in ["LD_PE_BIT", "ADD_PE_BIT", "ST_PE_BIT", "ROUTE_CM1"]:
            current_addr += 1
    ...
    for line in lines:
        ...
        if mnemonic == "LD_PE_BIT":
        ...
        elif mnemonic == "ROUTE_CM1":
            dim = get_val(ops[0])
            output.append(f"{0x7000 | (dim & 0x1):04X}")
        elif mnemonic == "LD_CU":
            rd = get_reg(ops[0])
            addr = get_val(ops[1])
            ...
            raise Exception(f"Unknown CM-1 mnemonic: {mnemonic}")
