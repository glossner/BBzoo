...
class Ibm1620Assembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 32
    num_gprs = 32
    num_fprs = 0
    ...
    def parse_reg(self, reg_str):
        ...
        raise Exception(f"Invalid IBM 1620 register operand: {reg_str}")
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "LOAD":
            if len(ops) != 2:
                raise Exception(f"LOAD instruction requires exactly 2 operands: {line}")
            ...
            if len(ops) != 2:
                raise Exception(f"STORE instruction requires exactly 2 operands: {line}")
            ...
            inst = (0x22 << 26) | (rd << 21) | (rs1 << 16) | (rs2 << 11)
            return [inst]

        else:
            raise Exception(f"Unknown IBM 1620 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled IBM 1620 Hex File")
        for val in values:
