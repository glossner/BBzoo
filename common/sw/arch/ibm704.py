...
class Ibm704Assembler(BaseAssembler, AccumulatorArchitecture, FixedLengthFormat, FixedPointOperations):
    word_width = 36
    address_width = 15
    has_link_bit = False
    op_code_width = 12
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic in ["LD", "ST", "ADD", "SUB"]:
            ...
            return [(op << 15) | (addr & 0x7FFF)]

        try:
            return [self.parse_numeric_or_symbol(line)]
        except ValueError:
            raise Exception(f"Unknown IBM 704 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled IBM 704 Hex File")
        for val in values:
