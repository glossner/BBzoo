...
class Ibm705Assembler(BaseAssembler, AccumulatorArchitecture, FixedLengthFormat, FixedPointOperations):
    word_width = 35
    address_width = 16
    has_link_bit = False
    op_code_width = 7
    instruction_width = 35
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic in ["LD", "ST", "ADD", "SUB"]:
            ...
            return [(op << 28) | (addr & 0xFFFF)]

        try:
            return [self.parse_numeric_or_symbol(line)]
        except ValueError:
            raise Exception(f"Unknown IBM 705 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled IBM 705 Hex File")
        for val in values:
            out.append(f"{val & 0x7FFFFFFFF:09X}")
        return "\n".join(out) + "\n"
