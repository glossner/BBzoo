...
class Ibm801Assembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 32
    num_gprs = 32
    num_fprs = 0
    ...
    def parse_reg(self, reg_str):
        ...
        raise Exception(f"Invalid IBM 801 register operand: {reg_str}")
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "L":
            if len(ops) != 2:
                raise Exception(f"L instruction requires exactly 2 operands: {line}")
            ...
            raise Exception(f"Unknown IBM 801 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled IBM 801 Hex File")
        for val in values:
