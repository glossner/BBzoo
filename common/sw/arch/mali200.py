...
class Mali200Assembler(BaseAssembler, VectorArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 16
    num_gprs = 4
    num_fprs = 0
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "VLD":
            if len(ops) != 2:
                raise Exception(f"VLD instruction requires exactly 2 operands: {line}")
            reg = ops[0].strip().upper()
            addr = self.parse_numeric_or_symbol(ops[1])
            reg_idx = int(reg[1:])
            if reg_idx < 0 or reg_idx >= 4:
                raise Exception(f"Invalid vector register index: {reg}")
            return [(0x10 << 24) | (reg_idx << 22), addr]

        elif mnemonic == "VST":
            if len(ops) != 2:
                raise Exception(f"VST instruction requires exactly 2 operands: {line}")
            reg = ops[0].strip().upper()
            addr = self.parse_numeric_or_symbol(ops[1])
            reg_idx = int(reg[1:])
            if reg_idx < 0 or reg_idx >= 4:
            ...
            if len(ops) != 3:
                raise Exception(f"VADD instruction requires exactly 3 operands: {line}")
            ...
            if any(idx < 0 or idx >= 4 for idx in [dest_idx, src1_idx, src2_idx]):
                raise Exception(f"Invalid vector register index in VADD: {line}")
            return [(0x20 << 24) | (dest_idx << 22) | (src1_idx << 18) | (src2_idx << 14)]

        else:
            raise Exception(f"Unknown Mali200 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled Mali200 Hex File")
        for val in values:
