...
class MultiflowAssembler(BaseAssembler):
    def __init__(self):
        super().__init__()
    ...
    def assemble_instruction(self, line):
        ...
        if upper.startswith("JMP "):
            target = self.parse_numeric_or_symbol(s[4:])
        ...
        slots = s.split(';')
        if len(slots) != 3:
            raise Exception(f"Invalid Multiflow VLIW instruction: {line}")
        ...
        if ctrl_str.upper() != "NOP":
            ...
            cop = cparts[0].upper()
            copcode = {"JMP": 1, "HLT": 2}[cop]
            target = self.parse_numeric_or_symbol(cparts[1].strip()) if cop == "JMP" else 0
