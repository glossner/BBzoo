...
class PowerpcAssembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 32
    num_gprs = 32
    num_fprs = 0
    ...
    def parse_reg(self, reg_str):
        ...
        raise Exception(f"Invalid PowerPC register operand: {reg_str}")
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "LWZ":
            ...
            raise Exception(f"Unknown PowerPC instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled PowerPC Hex File")
        for val in values:
