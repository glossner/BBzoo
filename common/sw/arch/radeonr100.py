...
class Radeonr100Assembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 16
    num_gprs = 4
    num_fprs = 0
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "LD":
        ...
        elif mnemonic == "TAPESTRY":
            if len(ops) != 4:
                raise Exception(f"TAPESTRY instruction requires exactly 4 operands: {line}")
            ...
            if any(idx < 0 or idx >= 4 for idx in [dest_idx, srcA_idx, srcB_idx, srcC_idx]):
                raise Exception(f"Invalid register index in TAPESTRY: {line}")
            return [(0x20 << 24) | (dest_idx << 22) | (srcA_idx << 18) | (srcB_idx << 14) | (srcC_idx << 10)]

        else:
            raise Exception(f"Unknown Radeonr100 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled Radeonr100 Hex File")
        for val in values:
