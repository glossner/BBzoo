...
class UcsdpAssembler(BaseAssembler, StackArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 16
    address_width = 16
    stack_depth = 16
    op_code_width = 8
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "PUSH":
            ...
            raise Exception(f"Unknown UCSD Pascal P-Machine instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled UCSD Pascal Hex File")
        for val in values:
