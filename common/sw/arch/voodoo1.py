...
class Voodoo1Assembler(BaseAssembler, GeneralRegisterArchitecture, VariableLengthFormat, FixedPointOperations):
    word_width = 32
    address_width = 16
    num_gprs = 4
    num_fprs = 0
    ...
    def assemble_instruction(self, line):
        ...
        if mnemonic == "LD":
            ...
            if any(idx < 0 or idx >= 4 for idx in [dest_idx, src1_idx, src2_idx]):
                raise Exception(f"Invalid register index in ADD: {line}")
            return [(0x20 << 24) | (dest_idx << 22) | (src1_idx << 18) | (src2_idx << 14)]

        else:
            raise Exception(f"Unknown Voodoo1 instruction: {line}")

    def format_output(self, values):
        out = []
        out.append("# Compiled Voodoo1 Hex File")
        for val in values:
