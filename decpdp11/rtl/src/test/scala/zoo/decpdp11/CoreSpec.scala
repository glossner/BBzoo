...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Pdp11Core"

  it should "execute MOV, ADD instructions correctly using register addressing modes" in {
    ...
    val hexFile = findWorkspaceFile("decpdp11/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Pdp11Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("4000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
