...
/**
 * NVIDIA GeForce 256 Core Processor.
 * A 32-bit GPU execution unit featuring register combiners.
 */
class Geforce256Core extends Module {
  ...
  // Sub-modules
  val decoder = Module(new Geforce256Decoder)
  ...
  switch(state) {
    ...
    is(sDECODE) {
      when(decoder.io.ctrl.is_hlt) {
        hltReg := true.B
        state  := sFETCH
      }.elsewhen(decoder.io.ctrl.is_combine) {
        val prod1 = gprs(decoder.io.ctrl.srcA_reg) * gprs(decoder.io.ctrl.srcB_reg)
        val prod2 = gprs(decoder.io.ctrl.srcC_reg) * gprs(decoder.io.ctrl.srcD_reg)
        gprs(decoder.io.ctrl.dest_reg) := prod1 + prod2
        state := sFETCH
    ...
    is(sSTORE_MEM) {
      ...
      io.mem.addr  := target_addr
      io.mem.write := true.B
      io.mem.wdata := gprs(decoder.io.ctrl.dest_reg) // in ST, dest_reg field is the source register
      when(io.mem.ready) {
        state := sFETCH
      }
    }
