...
class Core extends Module {
  ...
  val pe_masks   = RegInit(VecInit(Seq.fill(4)(true.B)))  // 1-bit mask/activity register G (active-high)
  ...
  switch(state) {
    ...
    is(sDecode) {
      ...
      when(decoder.io.hlt) {
        ...
        state := sCUReadAddr
      }.elsewhen(decoder.io.ld_pe_bit || decoder.io.add_pe_bit || decoder.io.st_pe_bit || decoder.io.ld_pe_mask) {
        peIdx := 0.U
        peSubState := sPE_Read
        state := sExecutePE
    ...
    is(sExecutePE) {
      when(peIdx < 4.U) {
        ...
        when(decoder.io.ld_pe_mask) {
          when(io.mem.ready) {
            reads := reads + 1.U
            pe_masks(peIdx) := io.mem.rdata(0).asBool
            peIdx := peIdx + 1.U
          }
        }.elsewhen(decoder.io.st_pe_bit) {
          when(!pe_masks(peIdx)) {
            // Masked: skip write
            peIdx := peIdx + 1.U
          }.elsewhen(peSubState === sPE_Read) {
            when(io.mem.ready) {
