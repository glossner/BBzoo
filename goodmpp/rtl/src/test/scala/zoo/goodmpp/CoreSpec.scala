...
class CoreSpec extends AnyFlatSpec with Matchers {
  ...
  it should "execute test_vector.hex program correctly" in {
    ...
    simulate(new Core) { c =>
      ...
      val hexFile = findWorkspaceFile("goodmpp/sw/test_vector.hex")
      ...
      mem(161) shouldBe 11 // C0
      mem(162) shouldBe 22 // C1
      mem(163) shouldBe 33 // C2
      mem(164) shouldBe 44 // C3
    }
  }
    ...
    assertThrows[Throwable] {
      simulate(new Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("8000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
