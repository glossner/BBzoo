...
/**
 * HP 3000 Core Processor.
 * A 16-bit stack-based core with stack cache registers.
 */
class Hp3000Core extends Module {
  ...
  // Sub-modules
  val decoder = Module(new Hp3000Decoder)
  ...
  // ALU inputs
  // HP 3000 is a stack machine: TOS + TOS-1 (data_out from StackMemory)
  alu.io.a  := tos
  ...
  switch(state) {
    ...
    is(sDECODE) {
      ...
      when(decoder.io.ctrl.is_hlt) {
        ...
        // Perform ADD: tos = tos + Stack(TOS-1)
        tos          := alu.io.out
        stack.io.pop := true.B
        state        := sFETCH
    ...
    is(sLOAD_MEM) {
      ...
      io.mem.write := false.B
      when(io.mem.ready) {
        // PUSH old tos to StackMemory if it is valid
        when(tos_valid) {
    ...
    is(sSTORE_MEM) {
      ...
      io.mem.wdata := tos
      when(io.mem.ready) {
        // Pop next element from StackMemory into tos
        stack.io.pop := true.B
        when(!stack.io.empty) {
