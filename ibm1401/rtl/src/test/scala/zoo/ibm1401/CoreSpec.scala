...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Ibm1401Core"

  it should "execute MC, A, and HLT correctly in memory-to-memory datapath" in {
    ...
    val hexFile = findWorkspaceFile("ibm1401/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Ibm1401Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("5000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
