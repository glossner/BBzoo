...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Ibm650Core"

  it should "execute LD, ADD, ST, and HLT correctly with next-instruction address chaining" in {
    ...
    val hexFile = findWorkspaceFile("ibm650/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Ibm650Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("500000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
