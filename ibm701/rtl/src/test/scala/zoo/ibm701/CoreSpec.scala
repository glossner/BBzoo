...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Ibm701Core"

  it should "execute LD, ST, ADD, SUB, and HLT correctly on 36-bit values" in {
    ...
    val hexFile = findWorkspaceFile("ibm701/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Ibm701Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("6000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
