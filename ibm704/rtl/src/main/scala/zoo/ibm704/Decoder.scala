...
class Ibm704CtrlSignals extends Bundle {
  val mem_read  = Bool()
  val mem_write = Bool()
  val is_ld     = Bool()
  val is_st     = Bool()
...
class Ibm704Decoder extends Module {
  val io = IO(new Bundle {
    ...
    val ctrl = Output(new Ibm704CtrlSignals)
    val addr = Output(UInt(15.W))
  })

  // Format: opcode (bits 15-26), address (bits 0-14)
  val opcode = io.inst(26, 15)
  io.addr   := io.inst(14, 0)
