...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Ibm704Core"

  it should "execute LD, ST, ADD, SUB, and HLT correctly on 36-bit values with 15-bit address width" in {
    ...
    val hexFile = findWorkspaceFile("ibm704/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Ibm704Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("30000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
