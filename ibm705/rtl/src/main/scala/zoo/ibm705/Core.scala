...
/**
 * IBM 705 Core Processor.
 * A 35-bit character-oriented accumulator CPU.
 */
class Ibm705Core extends Module {
  val io = IO(new Bundle {
    val mem = new SimpleMemIO(addrWidth = 16, dataWidth = 35)
    val hlt = Output(Bool())

    // Debug ports
    val pc_debug   = Output(UInt(16.W))
    val acc_debug  = Output(UInt(35.W))
  ...
  // Sub-modules
  val decoder = Module(new Ibm705Decoder)

  // Core Registers
  val pc     = RegInit(0.U(16.W))
  val acc    = RegInit(0.U(35.W))
  val hltReg = RegInit(false.B)
  val inst   = RegInit(0.U(35.W))
