...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Ibm705Core"

  it should "execute LD, ADD, ST, and HLT correctly on 35-bit values" in {
    ...
    val hexFile = findWorkspaceFile("ibm705/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Ibm705Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("60000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
