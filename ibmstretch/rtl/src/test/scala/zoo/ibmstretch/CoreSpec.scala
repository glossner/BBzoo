...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "IbmstretchCore"

  it should "execute LD, ADD, ST, LDX, and ADDX correctly using index registers" in {
    ...
    val hexFile = findWorkspaceFile("ibmstretch/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new IbmstretchCore) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("700000000000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
