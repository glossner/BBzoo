...
class CoreSpec extends AnyFlatSpec with Matchers {
  ...
  it should "execute test_vector.hex program correctly" in {
    ...
    simulate(new Core) { c =>
      ...
      val hexFile = findWorkspaceFile("illiac4/sw/test_vector.hex")
      val lines = Source.fromFile(hexFile).getLines()
        .map(_.trim)
        .filterNot(line => line.isEmpty || line.startsWith("#"))
        .map(line => java.lang.Long.parseLong(line, 16))
        .toArray
      ...
      mem(18) shouldBe 11 // C0
      mem(19) shouldBe 22 // C1
      mem(20) shouldBe 33 // C2
      mem(21) shouldBe 44 // C3
    }
  }
    ...
    assertThrows[Throwable] {
      simulate(new Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("d000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
