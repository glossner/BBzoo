...
class Intel3002Core extends Module {
  val io = IO(new Bundle {
    ...
    val regs_debug = Output(Vec(12, UInt(16.W)))
  ...
  val regs = RegInit(VecInit(Seq.fill(12)(0.U(16.W))))
  val decoder = Module(new Intel3002Decoder)
  ...
  // ALU connections
  alu.io.a := regs(decoder.io.rs1_addr)
  alu.io.b := regs(decoder.io.rs2_addr)
  ...
  switch(state) {
    ...
    is(sEXECUTE) {
      when(decoder.io.ctrl.mem_read) {
        io.mem.req := true.B
        io.mem.addr := Mux(opcode === 9.U, regs(decoder.io.rs1_addr), mar)
        io.mem.write := false.B
        when(io.mem.ready) {
          md := io.mem.rdata
          state := sWRITEBACK
        ...
        io.mem.req := true.B
        io.mem.addr := Mux(opcode === 10.U, regs(decoder.io.rs2_addr), mar)
        io.mem.write := true.B
        io.mem.wdata := regs(decoder.io.rs1_addr)
        when(io.mem.ready) {
          state := sFETCH
        }
        ...
        val rd_val = regs(decoder.io.rs1_addr)
        when(rd_val =/= 0.U) {
          pc := mar
        }
        state := sFETCH
        ...
        regs(decoder.io.rd_addr) := alu.io.out
        state := sFETCH
      }
    }
