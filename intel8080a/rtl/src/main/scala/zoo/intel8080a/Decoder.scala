...
class Intel8080aDecoder extends Module {
  ...
  switch(io.opcode) {
    is(0x3A.U) { io.ctrl.is_lda     := true.B } // LDA addr
    is(0x32.U) { io.ctrl.is_sta     := true.B } // STA addr
    is(0x47.U) { io.ctrl.is_mov_b_a := true.B } // MOV B, A (A to B)
    is(0x80.U) { io.ctrl.is_add_b   := true.B } // ADD B
    is(0x76.U) { io.ctrl.is_hlt     := true.B } // HLT
  }
}
