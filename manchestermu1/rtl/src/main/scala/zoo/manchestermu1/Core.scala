...
class ManchesterCore extends Module {
  ...
  switch(state) {
    ...
    is(sMEM_READ) {
      ...
      io.mem.write := false.B
      when(io.mem.ready) {
        when(decoder.io.ctrl.is_ldn) {
          acc := -io.mem.rdata
        }.elsewhen(decoder.io.ctrl.is_sub) {
          acc := acc - io.mem.rdata
        }.elsewhen(decoder.io.ctrl.is_jmp) {
          pc := io.mem.rdata(12, 0)
        }.elsewhen(decoder.io.ctrl.is_jpr) {
          pc := pc + io.mem.rdata(12, 0)
        }
        state := sFETCH
      }
    }
