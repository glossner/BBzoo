...
class Mc10800Core extends Module {
  val io = IO(new Bundle {
    ...
    val regs_debug = Output(Vec(2, UInt(16.W)))
  ...
  val decoder = Module(new Mc10800Decoder)
  ...
  // ALU connections
  alu.io.a := regs(decoder.io.rs1_addr(0))
  alu.io.b := regs(decoder.io.rs2_addr(0))
  ...
  switch(state) {
    ...
    is(sEXECUTE) {
      when(decoder.io.ctrl.mem_read) {
        io.mem.req := true.B
        io.mem.addr := Mux(opcode === 9.U, regs(decoder.io.rs1_addr(0)), mar)
        io.mem.write := false.B
        when(io.mem.ready) {
          md := io.mem.rdata
          state := sWRITEBACK
        ...
        io.mem.req := true.B
        io.mem.addr := Mux(opcode === 10.U, regs(decoder.io.rs2_addr(0)), mar)
        io.mem.write := true.B
        io.mem.wdata := regs(decoder.io.rs1_addr(0))
        when(io.mem.ready) {
          state := sFETCH
        }
        ...
        val rd_val = regs(decoder.io.rs1_addr(0))
        when(rd_val =/= 0.U) {
          pc := mar
        }
        state := sFETCH
        ...
        regs(decoder.io.rd_addr(0)) := alu.io.out
        state := sFETCH
      }
    }
