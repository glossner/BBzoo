...
class CoreSpec extends AnyFlatSpec with Matchers {
  ...
  it should "execute test_vector.hex program correctly" in {
    ...
    simulate(new Mc10800Core) { c =>
      ...
      val hexFile = findWorkspaceFile("mc10800/sw/test_vector.hex")
      ...
      var cycles = 0
      val maxCycles = 1500  // Higher cycle limit for memory-based loops
      var halted = false
      ...
      mem(65) shouldBe 11 // C0
      mem(66) shouldBe 22 // C1
      mem(67) shouldBe 33 // C2
      mem(68) shouldBe 44 // C3
    }
  }
    ...
    assertThrows[Throwable] {
      simulate(new Mc10800Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("b000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
