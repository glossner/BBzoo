...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Mos6502Core"

  it should "execute LDA, ADC, STA, CLC instructions correctly on 8-bit values" in {
    ...
    val hexFile = findWorkspaceFile("mos6502/sw/test_vector.hex")
    ...
    simulate(new Mos6502Core) { c =>
      ...
      while (cycles < maxCycles && !halted) {
        ...
        val dbgPc  = c.io.pc_debug.peek().litValue.toInt
        val dbgAcc = c.io.acc_debug.peek().litValue.toInt
        val dbgC   = c.io.c_debug.peek().litToBoolean

        // Optional debugging print (can be commented out if too verbose)
        // println(s"Cycle $cycles: PC=$dbgPc, Req=$req, Addr=$addr, Write=$write, Wdata=$wdata, Acc=$dbgAcc, Carry=$dbgC")
