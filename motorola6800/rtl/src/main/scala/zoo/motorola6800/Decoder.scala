...
class Motorola6800Decoder extends Module {
  ...
  switch(io.opcode) {
    is(0xB6.U) { io.ctrl.is_ldaa := true.B } // LDAA extended
    is(0xB7.U) { io.ctrl.is_staa := true.B } // STAA extended
    is(0xBB.U) { io.ctrl.is_adda := true.B } // ADDA extended
    is(0x3E.U) { io.ctrl.is_wai  := true.B } // WAI
  }
}
