...
class Core extends Module {
  ...
  val vSrc2   = Reg(UInt(3.W)) // Can hold 3-bit GPR index for scalar operand
  ...
  switch(state) {
    ...
    is(sDecode) {
      ...
      when(op === 3.U) { // HLT
        ...
        vTarget := rd(1, 0)
        state   := sVectorStore
      }.elsewhen(op === 1.U || op === 2.U) { // VADD / VSADD
        vIdx    := 0.U
        vTarget := rd(1, 0)
        vSrc1   := rs1(1, 0)
        vSrc2   := rs2
    ...
    is(sVectorArithmetic) {
      ...
      val elem2 = Mux(opCode === 1.U, vectorRegs(vSrc2(1, 0))(vIdx(1, 0)), regs(vSrc2))
      vectorRegs(vTarget)(vIdx(1, 0)) := elem1 + elem2
