...
/**
 * PowerPC Core Processor.
 * A 32-bit RISC core with 32 GPRs (R0-R31).
 */
class PowerpcCore extends Module {
  ...
  // Sub-modules
  val decoder = Module(new PowerpcDecoder)
  ...
  switch(state) {
    ...
    is(sDECODE) {
      ...
      when(decoder.io.ctrl.is_hlt) {
        ...
        state := sFETCH
      }.elsewhen(decoder.io.ctrl.opcode === 0x02.U || decoder.io.ctrl.opcode === 0x03.U) { // LWZ or STW
        state := sFETCH_ADDR
      }.otherwise {
        state := sFETCH
      }
