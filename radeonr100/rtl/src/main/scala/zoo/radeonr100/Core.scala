...
/**
 * ATI Radeon R100 Core Processor.
 * A 32-bit GPU execution unit featuring Pixel Tapestry blending logic.
 */
class Radeonr100Core extends Module {
  ...
  // Sub-modules
  val decoder = Module(new Radeonr100Decoder)
  ...
  switch(state) {
    ...
    is(sDECODE) {
      when(decoder.io.ctrl.is_hlt) {
        hltReg := true.B
        state  := sFETCH
      }.elsewhen(decoder.io.ctrl.is_tapestry) {
        gprs(decoder.io.ctrl.dest_reg) := gprs(decoder.io.ctrl.srcA_reg) * gprs(decoder.io.ctrl.srcB_reg) + gprs(decoder.io.ctrl.srcC_reg)
        state := sFETCH
