...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Radeonr100Core"

  it should "execute LD, ST, TAPESTRY, HALT instructions correctly to perform vector addition" in {
    ...
    val hexFile = findWorkspaceFile("radeonr100/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Radeonr100Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("1000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
