...
/**
 * SPARC Core Processor.
 * A 32-bit RISC core with 32 GPRs (R0-R31).
 */
class SparcCore extends Module {
  ...
  // Sub-modules
  val decoder = Module(new SparcDecoder)
  ...
  switch(state) {
    ...
    is(sDECODE) {
      when(decoder.io.ctrl.is_hlt) {
        ...
        state := sFETCH
      }.elsewhen(decoder.io.ctrl.opcode === 0x02.U || decoder.io.ctrl.opcode === 0x03.U) { // LD or ST
        state := sFETCH_ADDR
      }.otherwise {
        state := sFETCH
      }
