...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "StczebraCore"

  it should "execute LD, ADD, ST, and HLT correctly using functional bits" in {
    ...
    val hexFile = findWorkspaceFile("stczebra/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new StczebraCore) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("50000000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
