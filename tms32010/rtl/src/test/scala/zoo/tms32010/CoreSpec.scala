...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "Tms32010Core"

  it should "execute LAC, SACL, ADD, HALT instructions correctly to perform vector addition" in {
    ...
    val hexFile = findWorkspaceFile("tms32010/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new Tms32010Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("400", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
