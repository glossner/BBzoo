...
class CoreSpec extends AnyFlatSpec with Matchers {
  behavior of "ZuseZ1Core"

  it should "execute Pr, Ps, ADD, and MOV correctly on 22-bit values" in {
    ...
    val hexFile = findWorkspaceFile("zusez1/sw/test_vector.hex")
    ...
    assertThrows[Throwable] {
      simulate(new ZuseZ1Core) { c =>
        ...
        while (cycles < 15) {
          ...
          if (req) {
            c.io.mem.ready.poke(true.B)
            c.io.mem.rdata.poke(BigInt("70000", 16).U)
          } else {
            c.io.mem.ready.poke(false.B)
          }
          c.clock.step(1)
